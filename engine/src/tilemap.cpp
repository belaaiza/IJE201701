...
bool TileMap::draw(){
	...
	//INFO("MENINA X = " << m_camera->m_focus->physics.position.getX() << " MENINA Y = " << m_camera->m_focus->physics.position.getY());
	//INFO("CAMERA X = " << m_camera->getX() << " CAMERA Y = " << m_camera->getY());
	
	for(int layer = 0; layer < map_depth; layer++){
		renderLayer(layer, m_camera->getX(), m_camera->getY());
	}
