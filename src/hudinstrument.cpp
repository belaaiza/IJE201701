...
{
    if(m_player->getActiveHability()->name() == globals::banjo){
    ...
    else if(m_player->getActiveHability()->name() == globals::eletric_guitar){
        active_hud = huds[globals::eletric_guitar];
        huds[globals::banjo]->setState(Component::State::disabled);
        huds[globals::accordion]->setState(Component::State::disabled);
    }
    else if(m_player->getActiveHability()->name() == globals::accordion){
